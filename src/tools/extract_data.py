...
def plot_compound_throughput(test_results_dir):
    ...
    # Group perf data by protocol
    protocol_data = {}
    for perf in perf_point:
        protocol_data.setdefault(perf.protocol, []).append(perf)

    # Write per-protocol CSV files and one combined CSV
    with open(f"{test_results_dir}/throughput_latency_loss.csv", "w", encoding="utf-8") as f:
        for proto in protocols:
            proto_perfs = protocol_data.get(proto, [])
    ...
    # Write per-protocol markdown tables
